...
namespace Minsk.CodeAnalysis.Syntax
{
    internal sealed class Parser
    {
        ...
        {
            var openBraceToken = MatchToken(SyntaxKind.OpenSquareBraceToken);
            var nodesAndSeparators = ImmutableArray.CreateBuilder<SyntaxNode>();


            var parseNextParameter = true;
            while (parseNextParameter &&
                   Current.Kind != SyntaxKind.CloseSquareBraceToken &&
                   Current.Kind != SyntaxKind.EndOfFileToken)
            {
            ...
            var nodes =  new SeparatedSyntaxList<ExpressionSyntax>(nodesAndSeparators.ToImmutable());
            var closeBraceToken = MatchToken(SyntaxKind.CloseSquareBraceToken);

            return new ArrayExpressionSyntax(_syntaxTree, openBraceToken, nodes, closeBraceToken);
        }
        ...
        {
            var nameExpression = ParseNameExpression();
            var openBraceToken = NextToken();
            var expression = ParseExpression();
            var closeBraceToken = MatchToken(SyntaxKind.CloseSquareBraceToken);

            return new ArrayIndexExpressionSyntax(_syntaxTree, nameExpression, openBraceToken, expression, closeBraceToken);
        }
        ...
        private ExpressionSyntax ParsePrimaryExpression()
        {
            switch (Current.Kind)
            {
                ...
                case SyntaxKind.OpenSquareBraceToken:
