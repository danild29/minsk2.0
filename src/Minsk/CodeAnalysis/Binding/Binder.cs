...
namespace Minsk.CodeAnalysis.Binding
{
    internal sealed class Binder
    {
        ...
        private BoundExpression BindExpressionInternal(ExpressionSyntax syntax)
        {
            switch (syntax.Kind)
            {
                ...
                case SyntaxKind.ArrayIndexExpression:
                    return BindArrayIndexExpression((ArrayIndexExpressionSyntax)syntax);
        ...
        private BoundExpression BindArrayIndexExpression(ArrayIndexExpressionSyntax syntax)
        {
            ...
            if (boundExpression.Type != TypeSymbol.Int)
            {
                _diagnostics.ReportInvalidArrayExpressionStatement(syntax.Expression.Location, "array Index can't be not int");
            }
            var variable = BindExpression(syntax.NameExpression);

            if (variable == null)
                return boundExpression;

            return new BoundArrayIndexExpression(syntax, variable, boundExpression);
        }
        ...
        private BoundExpression BindArrayExpression(ArrayExpressionSyntax syntax)
        {
            ...
            return new BoundArrayExpression(syntax, boundArguments.ToImmutable());
        }
